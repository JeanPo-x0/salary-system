...
COLOR_PREGUNTA ="#C88EA7"         # mauve/rosa-morado
COLOR_RESUMEN = "italic #E0B8B8"    # rosa-crema pastel, solo para "---- RESUMEN DE EL DIA ----"
COLOR_SEPARADOR = "#A8967E"    # beige-dorado apagado, para las líneas ====
COLOR_DESPEDIDA =  "#BD1883"      # rosa final
...
def pedir_continuar():
    ...
    while True:
        console.print("\n¿Quiere hacer otro calculo? ( Si/No ) ", style=COLOR_PREGUNTA, end="")
        respuesta = input().lower().strip()
...
# Calculo 
    ...
    for i in registro:
        ...
        if i["Extras"] == 0:
            ...
            almuerzo_texto = f'Almorzó {i['Almuerzo']}h' if i["Almuerzo"] > 0 else '"No" almorzó.'
            console.print(f'---- RESUMEN DE EL DIA ----\n', style=COLOR_SUBTITULO)
            print(f"Dias: {i['Dia']}.\nLugar de trabajo: {i['Ubicacion']}.\nDescripcion del trabajo: {i['Descripcion']}.\nHoras de trabajo: {i['Horas de trabajo']:.2f}h\n{almuerzo_texto}\nTrabajo un total de {i['Extras']:.2f}h extras!!\n")
            console.print(f"El total del dia de {Nombre} es de {formato_colones(total_dia)}\n", style=COLOR_RESUMEN)
